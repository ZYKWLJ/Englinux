...
void execute_four_token(char *token[], int token_num, char *origin_str)
{
    ...
    if (parser_second_token(token[1], origin_str, &find_mod, word, subquerys) == 2)
    {
        ...
        for (int i = 0; subquerys[i]; i++)
        {
            ...
            int result_size = 0;
            // printf("word:%s\n", word);
            find(subquerys[i], library_index, find_mod, result, &result_size);
            display(library_index, find_mod, subquerys[i], result, result_size, firstcol_show, secondcol_show, thirdcol_show, fourthcol_show);
            // free(result);
        }
    }
    // 只查询英文
